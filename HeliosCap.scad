...
mid=24.6;    // flat mirror outer diameter
...
bm=3.5;        // beam margin. free space around beam
...
module cap(){
  ...
  difference(){ // cap baffle
    color("black")
    translate([0,0,h2/2+h1])cylinder(h=h2,d=mod+bm+w2*2,center=true,$fn=100);
    translate([0,0,h2/2+h1])cylinder(h=h2,d=mod+bm,center=true,$fn=100);  // baffle hollow cylinder considering w2 
